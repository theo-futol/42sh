...
## expect_stdout_match <command>  : Command will be executed on both 42sh
##                                  and tcsh and stdout must match

## expect_stderr_match <command>  : Command will be executed on both 42sh
##                                  and tcsh and stderr must match

## expect_env_match <command>     : Command will be executed on both 42sh
##                                  and tcsh and environment variables must match

## expect_pwd_match <command>     : Command will be executed on both 42sh
##                                  and tcsh and their PWD environment variables
##                                  must match

## expect_stdout_equals <command> : Command will be executed on both 42sh
##                                  and tcsh and stdout must match

## expect_stdout_equals <command> <value> : Command will be executed on 42sh
##                                          and its stdout must be equal to value

## expect_stderr_equals <command> <value> : Command will be executed on 42sh
##                                          and its stderr must be equal to value

## expect_exit_code <command> <code> : Command will be executed on 42sh
...
##                                    must be passed to 42sh using `env` command.
##                                    Not recommended with *_match commands.
##                                    WITH_ENV="-i" is equivalent to `env -i ./42sh`.
...
if [[ ! -f "./42sh" ]]; then
    cat <<EOF
./42sh does not exist.
It is required that a 42sh executable be in the same directory as this script in order to test it (possible means of satisfying this requirement include moving this script to a directory containing a 42sh executable).
EOF
    exit 84
fi
...
expect_signal_message_match()
{
    ...
    echo "-----"
    echo "Expectation: When executed program send a $1 signal ($signal_id), 42sh stderr must match with tcsh"
    echo "---"
    ...
    42SH_OUTPUT=$(echo "/tmp/__minishell_segv $without_core_dump $signal_id" | ./42sh 2>&1 1>/dev/null)
    EXIT2=$?

    DIFF=$(diff --color=always <(echo "$TCSH_OUTPUT") <(echo "$42SH_OUTPUT"))
    if [[ $DIFF != "" ]]; then
        echo "< tcsh    > 42sh"
        echo
        echo "$DIFF"
...
expect_env_match()
{
    ...
    42SH_OUTPUT="$(echo "$@"$'\n'"env" | env -i $SAMPLE_ENV $WITH_ENV ./42sh 2>&1 | clean_env)"
    DIFF=$(diff --color=always <(echo $TCSH_OUTPUT) <(echo $42SH_OUTPUT))
    if [[ $DIFF != "" ]]; then
        echo "< tcsh    > 42sh"
        echo
    ...
    EXIT1=$?
    echo "$@" | env $WITH_ENV ./42sh 2>&1 >/dev/null
    EXIT2=$?
...
expect_stdout_match()
{
    ...
    echo "---"
    DIFF=$(diff --color=always <(echo "$@" | tcsh 2>/dev/null) <(echo "$@" | env $WITH_ENV ./42sh 2>/dev/null))
    if [[ $DIFF != "" ]]; then
        echo "< tcsh    > 42sh"
        echo
        echo "$DIFF"
...
expect_stdout_equals()
{
    ...
    echo "---"
    DIFF=$(diff --color=always <(echo "$2") <(echo "$(echo "$1" | env $WITH_ENV ./42sh 2>/dev/null)"))
    if [[ $DIFF != "" ]]; then
        echo "< expect    > 42sh"
        echo
        echo "$DIFF"
...
expect_stderr_match()
{
    ...
    echo "---"
    DIFF=$(diff --color=always <(echo "$(echo "$@" | tcsh 2>&1 >/dev/null | clean_tcsh_stderr)") <(echo "$(echo "$@" | env $ENV_VAR ./42sh 2>&1 >/dev/null)"))
    if [[ $DIFF != "" ]]; then
        echo "< tcsh    > 42sh"
        echo
        echo "$DIFF"
    ...
    echo "$@" | tcsh &>/dev/null
    EXIT1=$?
    echo "$@" | env $WITH_ENV ./42sh &>/dev/null
    EXIT2=$?
...
expect_stderr_equals()
{
    ...
    echo "---"
    DIFF=$(diff --color=always <(echo "$2") <(echo "$(echo "$1" | env $WITH_ENV ./42sh 2>&1 >/dev/null)"))
    if [[ $DIFF != "" ]]; then
        echo "< expect    > 42sh"
        echo
        echo "$DIFF"
